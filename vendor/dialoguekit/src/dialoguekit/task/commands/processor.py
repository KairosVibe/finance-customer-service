from dialoguekit.domain.contexts import SystemTaskCanceledContext, TaskContext, SystemTaskInterruptedContext, \
...
from dialoguekit.task.commands.command import Command, StartFlowCommand, SetSlotsCommand, ResumedFlowCommand, \
    CancelFlowCommand
...
class CommandProcessor:
    ...
    def _start_flow(self,
        ...
        if activate_task is not None:
            ...
            self._prefill_slots_from_context(state, flow_list)
    ...
    def _prefill_slots_from_context(self, state: DialogueState, flow_list: FlowList):
        """从跨任务上下文预填新流程槽位（如已验证账户号自动复用）。"""
        ...
        if flow is None:
            return
        for slot_name in (flow.slots or {}):
            if slot_name in state.context:
                task.slots.setdefault(slot_name, state.context[slot_name])
